...
class Worker
{
    ...
    function exec_create_full()
    {
        ...
        $zip = new \depexor\Utils\Zip($filename);
        $zip->setZipFile($filename);
        $zip->setComment("depexor export of the userfiles folder and db.
				\n The depexor version at the time of export was {MW_VERSION}
    ...
    function create($filename = false)
    {
        ...
        $head = "\xEF\xBB\xBF";
        $head = "\n";
        //  $head = "/* depexor database exported on: " . date('l jS \of F Y h:i:s A') . " */ \n";
...
//        }
    ...
    function download($params)
    {
        ...
        if (is_file($filename)) {
            $dl = new \depexorPackages\Utils\System\Files();
