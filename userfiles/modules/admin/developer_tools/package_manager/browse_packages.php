...
$search_packages = [];
$composerClient = new \depexorPackages\Package\depexorComposerClient();
...
if ($search_packages and is_array($search_packages)) {
    foreach ($search_packages as $key => $item) {
        ...
        //if ($item['type'] != 'depexor-core-update') {
        ...
        //}
        if (isset($item['type']) && $item['type'] != 'depexor-core-update') {
...
if ($is_update_mode and isset($packages_by_type_with_update['depexor-core-update']) and !empty($packages_by_type_with_update['depexor-core-update'])) {
    $core_update = $packages_by_type_with_update['depexor-core-update'];
    unset($packages_by_type_with_update['depexor-core-update']);
    //$packages_by_type_with_update['depexor-core-update'] = array();
    //$packages_by_type_with_update['depexor-core-update'][] = $core_update;
}

$packages_by_type_reorder = $packages_by_type;
$packages_by_type = [];
$packages_by_type['depexor-template'] = [];
if(isset($packages_by_type_reorder['depexor-template'])){
    $packages_by_type['depexor-template'] = $packages_by_type_reorder['depexor-template'];
}


$packages_by_type['depexor-module'] = [];
if(isset($packages_by_type_reorder['depexor-module'])){
    $packages_by_type['depexor-module'] = $packages_by_type_reorder['depexor-module'];
}
...
<div class="card style-1 mb-3 <?php if ($from_live_edit): ?>card-in-live-edit<?php endif; ?>">
    ...
    <div class="card-body pt-3">
        ...
        <style>
            ...
            .package-image.package-depexor-template {
                width: 100%;
                height: 475px;
                cursor: -webkit-grab;
                cursor: -moz-grab;
        ...
        <script>
            $(document).ready(function () {
                $('.package-depexor-template').each(function () {
                    $(this)
                    ...
                    <?php foreach ($core_update as $pkkey => $pkitems): ?>
                        <?php foreach ($core_update as $key => $item): ?>
                            <div class="col-12 col-md-6 mb-4">
                                ...
                                $view = new \depexorPackages\View\View($view_file);
                                    ...
                                    <?php if ($pkitems) : ?>
                                        <div class="row">
                                            <?php foreach ($pkitems as $key => $item): ?>
                                                <div class="col-12 col-sm-6 col-md-<?php print $item['type'] === 'depexor-module' ? '6' : '12'; ?> col-lg-<?php print $item['type'] === 'depexor-module' ? '4' : '6'; ?> mb-4 package-col-<?php print $item['type']; ?>">
                                                    <?php
                                                    $view_file = __DIR__ . '/partials/package_item.php';
                                                    $view = new \depexorPackages\View\View($view_file);
                                                    $view->assign('item', $item);
