...
class Backup
{
    ...
    public static function bgworker_restore($params)
    {
        ...
        $api = new \depexorPackages\Backup\BackupManager();
    ...
    public static function bgworker()
    {
        ...
        if (!defined('MW_BACKUP_BG_WORKER_STARTED')) {
            ...
            $backup_api = new \depexorPackages\Backup\BackupManager();
    ...
    public function exec_create_full()
    {
        ...
        $zip = new \depexorPackages\Utils\Zip($filename);
        $zip->setZipFile($filename);
        $zip->setComment("depexor backup of the userfiles folder and db.
                \n The depexor version at the time of backup was {MW_VERSION}
...
//                $stmt = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x80-\x9F]/u', '', $stmt);
    ...
    public function create($filename = false)
    {
        ...
        $head = '/* depexor database backup exported on: ' . date('l jS \of F Y h:i:s A') . " */ \n";
