...
namespace depexorPackages\LegacyBackup;

api_expose_admin('depexorPackages\LegacyBackup\Import\delete');
//api_expose_admin('depexorPackages\LegacyBackup\Import\create');
api_expose_admin('depexorPackages\LegacyBackup\Import\download');
//api_expose_admin('depexorPackages\LegacyBackup\Import\create_full');
api_expose_admin('depexorPackages\LegacyBackup\Import\move_uploaded_file_to_import');
api_expose_admin('depexorPackages\LegacyBackup\Import\restore');
api_expose_admin('depexorPackages\LegacyBackup\Import\export');
api_expose_admin('depexorPackages\LegacyBackup\Import\batch_process');
...
// Now that we're at the right depth, hop to the next "[item name]" until the end of tree/file.
    ...
    {
        must_have_access();
        $target_url = 'http://api.depexor.com/service/xls2csv/index.php';
        $filename = str_replace('..', '', $filename);
        ...
        if ($err == true) {
            return array('error' => 'Could not contact the depexor remote server to parse the Excel file. Please try uploading a CSV file.');
        } else {
            if (isset($result['result'])) {
                $url = $result['result'];
...
class SimpleXmlStreamer extends \depexorPackages\Utils\lib\XmlStreamer
{
    ...
    {
        $import = mw('depexorPackages\LegacyBackup\Import')->batch_save($this->content_items);
