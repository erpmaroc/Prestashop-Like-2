...
mw.admin_backup = {
	create : function(selector){
	 ...
	 $.post(mw.settings.api_url+'depexorPackages/LegacyBackup/Backup/create', false ,
	 	function(msg) {
	...
	create_template_export : function(){
	 ...
	 $.post(mw.settings.api_url+'depexor/Utils/Backup/create_template_export', false ,
	 	function(msg) {
	...
	restore : function(src,loading_element){
		if (confirm("Are you sure you want to restore this backup? All existing content will be replaced!")) {
		   ...
		   mw.notification.success("Backup restoration started",17000);
				data = {}
				data.id=src;
				$.post(mw.settings.api_url+'depexorPackages/LegacyBackup/Backup/restore', data ,
					function(msg) {
	...
	move_uploaded_file_to_backup : function(src){
		...
		data = {}
		data.src=src;
		$.post(mw.settings.api_url+'depexorPackages/LegacyBackup/Backup/move_uploaded_file_to_backup', data ,
			function(msg) {
				mw.reload_module('admin/backup/manage');
				mw.notification.msg(msg);
			});
	...
	create_full : function(selector){
		...
		$.post(mw.settings.api_url+'depexorPackages/LegacyBackup/Backup/create_full', false ,
			function(msg) {
				mw.reload_module('admin/backup/manage');
				mw.notification.msg(msg);
			});
	...
	remove : function($id, $selector_to_hide){
        ...
        mw.tools.confirm(mw.msg.del, function(){
      		...
      		$.post(mw.settings.api_url+'depexorPackages/LegacyBackup/Backup/delete', data ,
      			function(resp) {
      			 //mw.reload_module('admin/backup/manage');
