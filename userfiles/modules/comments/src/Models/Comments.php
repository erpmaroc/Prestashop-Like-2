...
namespace depexor\Comments\Models;

use Illuminate\Support\Facades\Notification;
use depexorPackages\Comment\Events\NewComment;
use depexorPackages\Comment\Notifications\NewCommentNotification;
use depexorPackages\Database\Crud;
use depexor\Utils\Http;
...
class Comments extends Crud
{
    ...
    public function save($data)
    {
        ...
        if (!isset($data['id']) and isset($data['comment_body'])) {
           ...
           /* $notif = array();
            ...
            $newComment = \depexorPackages\Comment\Comment::where('id',$saved_data_id)->first();
           if ($newComment) {
        ...
        if (isset($get_comment['is_subscribed_for_notification']) && isset($get_comment['is_sent_email'])) {
        	...
        	if ($get_comment['action'] == 'publish' && $get_comment['is_subscribed_for_notification'] == 1 && $get_comment['is_sent_email'] == 0) {
        		...
        		if (is_numeric($saved_data_id)) {
        			$emailJob = (new  \depexor\Comments\Jobs\JobSendMailNotificationOnComment($saved_data_id))->onQueue('processing');
    ...
    private function __report_for_spam($comment_id)
    {
        ...
        $report_url = 'https://spamchecker.depexorapi.com/';
        ...
        if ($comment) {
            ...
            $http = new \depexorPackages\Utils\Http\Http();
            $http->url($report_url);
            $http->set_timeout(10);
