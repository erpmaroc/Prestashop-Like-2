...
use depexorPackages\Core\Events\AbstractResourceWasUpdated;
...
class MenuManager
{
    ...
    public function menu_item_save($data_to_save)
    {
        ...
        if (isset($data_to_save['id']) && $data_to_save['id'] > 0) {
            $saveMenu = \depexorPackages\Menu\Menu::where('id', $data_to_save['id'])->first();
        }

        if ($saveMenu == null) {
            $saveMenu = new \depexorPackages\Menu\Menu();
        }
