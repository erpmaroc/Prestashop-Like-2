...
use depexorPackages\Package\depexorComposerClient;
...
class UpdateManager
{
    ...
    private $remote_api_url = 'http://update.depexorapi.com/';
    private $remote_url = 'http://update.depexorapi.com/';
    ...
    {
        return new \depexorPackages\Utils\Http\Http();
    }
    ...
    public function collect_local_data()
    {
        ...
        $data['update_channel'] = \Config::get('depexor.update_channel');
        $data['last_update'] = \Config::get('depexor.updated_at');
    ...
    public function marketplace_link($params = false)
    {
        ...
        } elseif ($this->app->make('config')->get('depexor.marketplace_provider_id')) {
            $params['marketplace_provider_id'] = $this->app->make('config')->get('depexor.marketplace_provider_id');
        }
...
//        }
    ...
    public function check($skip_cache = false)
    {
        ...
        $update_channel = Config::get('depexor.update_channel');
...
//                    $filesystem = new \depexorPackages\Utils\Files();
...
//                    //$checksum['userfiles/modules/depexor'] = $filesystem->md5_dir(mw_includes_path());
...
//        }
    ...
    public function install_version($new_version, $on_step = false)
    {
        ...
        if (isset($result['core_update'])) {
            ...
            if (!$on_step or $on_step > 1) {
                ...
                if (is_file($dl_file)) {
                    $unzip = new \depexorPackages\Utils\Unzip();
    ...
    public function post_update($version = false)
    {
        ...
        if (mw_is_installed()) {
            ...
            $system_refresh = new \depexorPackages\Install\DbInstaller();
         ...
         //   scan_for_elements(['no_cache'=>true,'reload_modules'=>true,'cleanup_db'=>true]);
            ...
            if ($version != false) {
                Config::set('depexor.version', $version);
                Config::set('depexor.updated_at', @date("Y-m-d H:i:s"));
                Config::save('depexor');
            }
        }
        return true;
    ...
    public function call($method = false, $post_params = false)
    {
        ...
        if ($post_params != false and is_array($post_params)) {
            ...
            $curl = new \depexorPackages\Utils\Http\Http($this->app);
    ...
    public function send_anonymous_server_data($params = false)
    {
        ...
        $params['function_name'] = 'send_lang_form_to_depexor';
