...
function _e($k, $to_return = false, $replace = [])
{
    ...
    $trans = str_ireplace('{{app_name}}', 'depexor', $trans);
...
/**
 * Send your language translation to depexor.
 *
 * @internal its used via ajax in the admin panel under Settings->Language
 */

api_expose_admin('send_lang_form_to_depexor', function ($data) {
    if (is_admin() == true) {
        $lang = current_lang();
        $send = array();
...
function get_flag_icon($locale) {
    return \depexorPackages\Translation\LanguageHelper::getLanguageFlag($locale);
}
