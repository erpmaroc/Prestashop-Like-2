...
api_expose_admin('media_library/search', function ($data) {
    ...
    $search = array();
    $unsplash = new \depexorPackages\Utils\Media\Adapters\Unsplash();
...
\Illuminate\Support\Facades\Route::get('/api/image-generate-tn-request/{cache_id}', function ($mediaId) {
    ...
    $check = \depexorPackages\Media\Models\MediaThumbnail::where('id', $mediaId)->first();
...
api_expose('queue_dispatch1', function () {
    ...
    $job = Queue::push('\depexorPackages\Utils\Import', ['export' => '']);
    ...
    $job = mw('\depexorPackages\Utils\Import', ['export' => '']);
    dispatch($job)->onQueue('high');
