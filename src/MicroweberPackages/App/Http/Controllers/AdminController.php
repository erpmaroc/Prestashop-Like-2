...
namespace depexorPackages\App\Http\Controllers;
...
use Auth;
use depexorPackages\Install\Http\Controllers\InstallController;
use depexorPackages\User\Models\User;
use depexorPackages\View\View as depexorView;
...
class AdminController extends Controller
{
    ...
    public function render() {
        ...
        if (!$is_installed) {
            ...
            $config_version = Config::get('depexor.version');
            if ($config_version != MW_VERSION) {
                $this->app->update->post_update(MW_VERSION);
            }
        }

        $force_https = \Config::get('depexor.force_https');
...
//        }
    ...
    {
        $serviceConfig = Config::get('services.depexor');
        if (!$serviceConfig) {
            return false;
        }
    ...
    private function registerMwClient()
    {
        ...
        $client = new \Guzzle\Service\Client('https://login.depexor.com/api/v1/client/');
        ...
        if (200 == $response->getStatusCode()) {
            ...
            Config::set('services.depexor', $body);
            Config::save(array('depexor', 'services'));

            save_option([
                'option_value' => 'y',
                'option_key' => 'enable_user_depexor_registration',
                'option_group' => 'users',
            ]);
        } else {
