...
use depexorPackages\App\Http\Middleware\ApiAuth;
use depexorPackages\App\Http\Middleware\SameSiteRefererMiddleware;
use depexorPackages\App\Managers\Helpers\VerifyCsrfTokenHelper;
use depexorPackages\App\Traits\LiveEditTrait;
...
class FrontendController extends Controller
{
    ...
    public function index()
    {
        ...
        if (\Config::get('depexor.force_https') && !is_cli() && !is_https()) {
...
//            $l = str_ireplace('<head>', '<head>' . $apijs_settings_script, $l);
    ...
    public function sitemapxml()
    {
        ...
        if ($updateSitemap) {
            $map = new \depexor\Utils\Sitemap($sm_file);
            $map->file = $sm_file;
