<?php

namespace depexorPackages\Form\Elements;

class TextAreaOption extends TextArea
{
    protected $model;
    protected $optionKey;
    protected $optionGroup;

    public $attributes = [
        'class'=>'form-control mw_option_field',
    ];

    public function __construct($optionKey, $optionGroup) {

        $this->optionKey = $optionKey;
        $this->optionGroup = $optionGroup;

        $this->setName($optionKey);
        $this->setAttribute('option-group', $optionGroup);

        $this->model = \depexorPackages\Option\Models\ModuleOption::where('option_key', $this->optionKey)->where('option_group', $this->optionGroup)->first();
        if ($this->model) {
            $this->value($this->model->option_value);
        }
    }
}
