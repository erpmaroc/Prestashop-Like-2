...
namespace depexorPackages\Install\Http\Controllers;
...
use depexorPackages\Package\ComposerUpdate;
use depexorPackages\View\View;
use depexorPackages\Install;
...
class InstallController extends Controller
{
    ...
    public function index($input = null)
    {
        ...
        if ($is_installed) {
            return 'depexor is already installed!';
        }
        ...
        $allowed_configs = array('database', 'depexor');
        ...
        if (isset($input['make_install'])) {
            ...
            if (defined('MW_VERSION')) {
                Config::set('depexor.version', MW_VERSION);
            }

            if (isset($input['default_template']) and $input['default_template'] != false) {
                Config::set('depexor.install_default_template', $input['default_template']);
            }
            if (isset($input['with_default_content']) and $input['with_default_content'] != false) {
                Config::set('depexor.install_default_template_content', 1);
            }

            if (!isset($input['developer_mode'])) {
                Config::set('depexor.compile_assets', 1);
                Config::set('depexor.disable_model_cache', 0);
            }
            if (isset($input['clean_pre_configured'])) {
                Config::set('depexor.pre_configured', null);
                Config::set('depexor.pre_configured_input', null);
            }

            if (isset($input['admin_url'])) {
                Config::set('depexor.admin_url', $input['admin_url']);
            }
            ...
            if (isset($input['site_lang'])) {
                ...
                Config::set('depexor.site_lang', $input['site_lang']);
            }
            ...
            if ($config_only) {
                Config::set('depexor.pre_configured', 1);
                Config::set('depexor.pre_configured_input', $input);
            } else {
                ...
                if (!$install_step or $install_step == 7) {
                    if (isset($input['site_lang'])) {
                        ...
                        $selected_template = Config::get('depexor.install_default_template');
...
//                        TranslationPackageInstallHelper::installLanguage($input['site_lang']);
                ...
                if (isset($input['admin_password']) && strlen($input['admin_password'])) {
                    if (isset($input['admin_email']) or isset($input['admin_username'])) {
                        ...
                        Config::set('depexor.has_admin', 1);
                    }
                }

                $this->log('Saving ready config');

                Config::set('depexor.is_installed', 1);
            ...
            if (Config::get('depexor.has_admin') and !is_cli() and isset($admin_user_id)) {
        ...
        if (Config::get('depexor.pre_configured')) {
            $viewData['pre_configured'] = Config::get('depexor.pre_configured');
            if (Config::get('depexor.pre_configured_input') and is_array(Config::get('depexor.pre_configured_input'))) {
                $viewData['config'] = array_merge($viewData['config'], Config::get('depexor.pre_configured_input'));
            }
        }
        $layout->set($viewData);
        ...
        $is_installed = mw_is_installed();
        if ($is_installed) {
            App::abort(403, 'Unauthorized action. depexor is already installed.');
        }
    ...
    {
        $um = new \depexorPackages\App\Managers\UpdateManager(app());
        ...
        try {
            $http->url('https://installreport.services.depexorapi.com')->set_timeout(10)->post($postData);
        } catch (\Exception $e) {
    ...
    private function _get_market_templates_for_install_screen()
    {
        ...
        $results = $runner->searchPackages(['search_by_type' => 'depexor-template']);
        if ($results) {
