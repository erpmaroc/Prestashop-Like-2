...
namespace depexorPackages\User;
...
use depexorPackages\App\LoginAttempt;
use depexorPackages\User\Http\Controllers\UserLoginController;
use depexorPackages\User\Http\Requests\LoginRequest;
use depexorPackages\User\Http\Resources\UserResource;
use depexorPackages\User\Models\User;
use depexorPackages\User\Socialite\depexorProvider;
use depexorPackages\Utils\ThirdPartyLibs\DisposableEmailChecker;
...
class UserManager
{
    ...
    public function loginOLD($params)
    {
        ...
        if ($ok) {
            if (function_exists('user_ip') and (Auth::user()->is_admin) == 1) {
                $allowed_ips = config('depexor.admin_allowed_ips');
                if ($allowed_ips) {
    ...
    public function picture($user_id = false)
    {
        ...
        return modules_url() . 'depexor/api/libs/mw-ui/assets/img/no-user.png';
    }
            ...
            if ($register_email_enabled == true) {
                ...
                if ($register_email_content != false and trim($register_email_subject) != '') {
                    ...
                    if (isset($to) and (filter_var($to, FILTER_VALIDATE_EMAIL))) {
                        ...
                        $sender = new \depexorPackages\Utils\Mail\MailSender();
...
//                    Session::save();
    ...
    public function socialite_config($provider = false)
    {
        ...
        if (get_option('enable_user_depexor_registration', 'users') == 'y') {
            $svc = Config::get('services.depexor');
            if (!isset($svc['client_id'])) {
                Config::set('services.depexor.client_id', get_option('depexor_app_id', 'users'));
            }
            if (!isset($svc['client_secret'])) {
                Config::set('services.depexor.client_secret', get_option('depexor_app_secret', 'users'));
            }
            if (!isset($svc['redirect'])) {
                Config::set('services.depexor.redirect', $callback_url);
            }
            $this->socialite->extend('depexor', function ($app) {
                $config = $app['config']['services.depexor'];

                return $this->socialite->buildProvider(depexorProvider::class, $config);
            });
        }
    }
    ...
    public function get_shipping_address()
    {
        ...
        if ($this->is_logged()) {
            $findCustomer = \depexorPackages\Customer\Models\Customer::where('user_id', Auth::id())->first();
            if ($findCustomer) {
                $findAddressShipping = \depexorPackages\Customer\Models\Address::where('type', 'shipping')->where('customer_id', $findCustomer->id)->first();
    ...
    private function __check_id_has_ability_to_edit_user($user_id)
    {
        ...
        $disable_edit_users = Config::get('depexor.users_disable_edit');
