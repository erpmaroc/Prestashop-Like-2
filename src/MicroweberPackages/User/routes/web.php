...
// Admin web
Route::prefix(ADMIN_PREFIX)->middleware(['admin'])->namespace('\depexorPackages\User\Http\Controllers')->group(function () {
    Route::get('login', 'UserLoginController@index')->name('admin.login')->middleware(['allowed_ips']);
});

// Public user
Route::namespace('\depexorPackages\User\Http\Controllers')->group(function () {
    Route::get('login', 'UserLoginController@loginForm')->name('login');
});

Route::namespace('\depexorPackages\User\Http\Controllers')->middleware(['web'])->group(function () {

    Route::get('email/verify/{id}/{hash}', 'UserVerifyController@verify')->name('verification.verify')
        ->middleware([\depexorPackages\User\Http\Middleware\UserValidateEmailSignature::class]);
